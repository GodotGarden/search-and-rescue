...
If your environment has no display (common for sandboxed agents), you can't exercise behavior interactively — fall back to `godot --headless --path . --check-only --script <path>` (syntax) and `godot --headless --path . --import` (catches scene load errors), and where a numeric claim matters (jump height, fall time, a spring's settle time), write a throwaway `SceneTree`-script probe that drives the actual scene/body through `_physics_process` and logs the values you're changing — a before/after comparison run this way is far stronger evidence than reasoning about the constants alone. Write the probe to a scratch location, never into the repo, and say plainly that this is what you did rather than implying an interactive playtest happened.
