...
- **Know the rig's ground-contact limit.** Joint rotation alone cannot put a kneeling/seated/prone pose's contact point (knee, hip, torso) exactly on the ground — the rig has a fixed root position and fixed proportions, so literal floor contact needs a root offset, which doesn't exist as a mechanism yet. Until it does, treat ground-contact poses as joint-rotation approximations (a readable silhouette, not verified floor contact) and say so explicitly. Adding root-offset support is a cross-domain question, not a unilateral one — it needs to coordinate with collider height and camera-pivot height, both simulation-physics' territory. The `root_offset`/`root_rotation` fields already anticipated in the pose library appendix of `docs/specifications/procedural-preview-lab.md` are where this is meant to land.
...
The preview lab needs a real rendering context to produce pixels — it does not work under `--headless`. If your environment has no display at all (common for sandboxed agents), it's simply unavailable; fall back to `godot --headless --path . --check-only --script <path>` (syntax) and `godot --headless --path . --import` (catches scene/resource load errors), and say plainly that this is a structural check, not a visual one — don't imply a look you didn't actually get.
